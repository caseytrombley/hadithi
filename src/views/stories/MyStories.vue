<template>
  <layout-document :is-loading="loading">
    <template #header>
      <search-bar
          place-holder="Search for stories"
...
export default {
  ...
  computed: {
    ...
    userPermissions: function () {
      ...
      return {
        CAN_READ: permissions["CAN_READ_STORY"],
        CAN_EDIT: permissions["CAN_EDIT_STORY"],
        CAN_PUBLISH: permissions["CAN_PUBLISH_STORY"],
        CAN_DELETE: permissions["CAN_DELETE_STORY"]
      };
    },
  },
  watch: {
  ...
  methods: {
    ...
    async getItems() {
      let myStories = await StoryService.getStoriesByEmail(this.creatorEmail);
      myStories = booleanFieldSort(myStories, "isFree");
      return timestampFieldSort(myStories, "updatedAt");
    },
    ...
    async handlePublish(document) {
      if (confirm("Are you sure you want to publish?")) {
        await StoryService.updateStory(document.id, {status: "published", published: true, updatedAt: new Date()});
        await this.setDefaultData();
      }
    },
    async handleUnPublish(document) {
      if (confirm("Are you sure you want to unPublish?")) {
        await StoryService.updateStory(document.id, {status: "review", published: false, updatedAt: new Date()});
        await this.setDefaultData();
      }
    },
    async handleDelete(document) {
      if (confirm("Are you sure you want to delete?")) {
        await StoryService.deleteStory(document.id);
        this.$router.go(0);
      }
    },
  },
